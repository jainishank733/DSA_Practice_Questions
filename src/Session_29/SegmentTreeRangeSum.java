...
public class SegmentTreeRangeSum {
    ...
    private static void build(int[] arr, int l, int r, int[] segTree, int ci) {
        ...
        segTree[ci] = segTree[2 * ci] + segTree[2 * ci + 1];
    }

    private static int sumQuery(int[] segTree, int cl, int cr, int ql, int qr, int ci, int[] lazy) {
        if (lazy[ci] != 0) {
        ...
        int mid = (cl + cr) / 2;
        int leftMin = sumQuery(segTree, cl, mid, ql, qr, 2 * ci, lazy);
        int rightMin = sumQuery(segTree, mid + 1, cr, ql, qr, 2 * ci + 1, lazy);
    ...
    public static void main(String[] args) {
        ...
        System.out.println(sumQuery(segTree, 0, n - 1, 2, 4, 1, lazyTree));
